...
abstract class Generator<T> {
    ...
    def String generateMethodSignature(MethodModel methodModel) '''
        «methodModel.getVisibility().getCode()»«IF methodModel.isStatic()» static«ENDIF»«IF methodModel.isAbstract()» abstract«ENDIF»«IF methodModel.hasReturnType()» «methodModel.getReturnType()»«ELSEIF !methodModel.isConstructor()» void«ENDIF» «methodModel.getName()»(«FOR AttributeModel attribute : methodModel.getParameters() SEPARATOR ', '»«attribute.getType()» «attribute.getName()»«ENDFOR»)
    '''
